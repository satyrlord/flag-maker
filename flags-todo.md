...
and `src/templates.ts` against the reference flag on Wikipedia. It is designed for parallel work
...
| 215 | England | `england` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 216 | Scotland | `scotland` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 217 | Wales | `wales` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 218 | Northern Ireland | `northern_ireland` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 219 | Catalunya | `catalunya` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 220 | Euskadi | `euskadi` | not started | | | State-level template entry added with the dedicated State Level template group. |
| 221 | Bavaria | `bavaria` | not started | | | State-level template entry added with the dedicated State Level template group. |
